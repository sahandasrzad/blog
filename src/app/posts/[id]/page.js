...
export default async function PostDetail({ params }) {
  ...
  try {
    ...
    // Handle the error appropriately, maybe redirect or show an error message
    console.error('Error fetching post:', error);
    return <p className="text-red-500">خطا در دریافت اطلاعات</p>;
  }
      ...
      <div className="w-full h-full p-4">
        <div className="px-4 mt-12">
          ...
          <div className="w-full mt-5 flex flex-row h-24">
            <a href={`/update/${post._id}`} className={`border-orange-500 dark:border-gray-300 text-orange-500 dark:text-gray-300 mt-auto border px-2 py-1 rounded-md w-auto text-sm`}>
              ویرایش
            </a>
          </div>
        </div>
